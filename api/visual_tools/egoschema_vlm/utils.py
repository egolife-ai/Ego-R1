...
# def get_video_length_cv2(video_path: str) -> float:
...
#         logging.error(f"Error getting video length with OpenCV: {str(e)}")
#         return 0
...
def map_hhmmssff_to_range(hhmmssff: str,video_end_time:str) -> str:
    fps = 30  # 固定帧率为30
    ...
    input_frames = to_frames(hhmmssff)
    max_frames = to_frames(video_end_time)  # 3分钟 = 180秒 = 5400帧

    mapped_frames = input_frames % max_frames
...
def locate_video_url(timestamp: Annotated[str, "The timestamp of the video to answer the question. The format should be DAYX_HHMMSSFF (X is the day number, HHMMSS is the hour, minute, second, and FF is the frame number(00~19))"], data_dir: Annotated[str, "The directory of all videos"] = "/home/data2/sltian/code/Ego-R1_dev/EgoLife") -> str:
    ...
    try:
        ...
        time_part=scale_time_proportional(time_part,"00030000")
        day = day_part[3:]
