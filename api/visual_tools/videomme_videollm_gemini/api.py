...
# Load configuration
config = load_config()
DATA_DIR = config['videomme']['videollm']['data_dir']
GEMINI_API_KEYS = config['videomme']['videollm']['gemini_api_keys']
PORT = config['videomme']['videollm']['gemini_port']
        ...
        if isinstance(request, list):
            ...
            for idx, item in enumerate(request):
                if item is None or item == "" or (isinstance(item, str) and item.strip() == ""):
                    ...
                    tasks.append("PENDING")  # placeholder
...
def start_server( port: int = 7060):
