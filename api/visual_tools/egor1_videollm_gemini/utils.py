...
def config_videos(
    ...
    if len(file_paths) == 1:
        ...
        if trim_s==trim_e:
            raise ValueError("Video length should greater than 1 second.")
        trim_video(
            file_s,
            concat_video,
...
def locate_videos(start_timestamp, end_timestamp, data_dir="./EgoLife", identity="A1", cache_dir=None):
    ...
    if abs(calculate_time_diff(end_timestamp, start_timestamp)) > 600:
        ...
        start_url, start_exact_match = locate_video_url(timestamp=start_timestamp, identity=identity, data_dir=data_dir)
        end_url, end_exact_match = locate_video_url(timestamp=end_timestamp, identity=identity, data_dir=data_dir)
        print(f"The start video is: {start_url}, the end video is: {end_url}")
