...
def locate_videos(start_timestamp, end_timestamp, data_dir="./EgoLife", cache_dir=None):
    ...
    start_time = "00" + start_timestamp.split("_")[-1][2:]
    end_time = "00" + end_timestamp.split("_")[-1][2:]
    ...
    if start_time > clip_end_time:
        ...
        video_paths = [
            os.path.join(data_dir, f) for f in sorted(os.listdir(data_dir)) if f.endswith(".mp4")
        ]
        return video_paths, False,"DAY1_00000000"
    elif end_time > clip_end_time:
