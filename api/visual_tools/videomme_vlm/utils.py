...
def locate_videos(start_timestamp: Annotated[str, "The start timestamp of the video to answer the question. The format should be DAYX_HHMMSSFF (X is the day number, HHMMSS is the hour, minute, second, and FF is the frame number(00~19))"], end_timestamp: Annotated[str, "The end timestamp of the video to answer the question. The format should be DAYX_HHMMSSFF (X is the day number, HHMMSS is the hour, minute, second, and FF is the frame number(00~19))"], data_dir: Annotated[str, "The directory of all videos"] = "./EgoLife", identity: Annotated[str, "The identity of the person in the video"] = "A1") -> str:
    ...
    if abs(calculate_time_diff(end_timestamp, start_timestamp)) > 600:
        ...
        start_url, start_exact_match = locate_video_url(timestamp=start_timestamp)
        end_url, end_exact_match = locate_video_url(timestamp=end_timestamp)
...
def scale_time_proportional(input_time_hhmmssff: str, target_duration_hhmmssff: str) -> str:
    """
    将一个在24小时内的时间点，按比例缩放到一个指定的目标时长范围内。

    Args:
        input_time_hhmmssff (str): 来源时间点，基于24小时制 (e.g., "12000000")。
        target_duration_hhmmssff (str): 希望映射到的目标总时长 (e.g., "00030000" for 3 minutes)。
...
def locate_video_url(timestamp: Annotated[str, "The timestamp of the video to answer the question. The format should be DAYX_HHMMSSFF (X is the day number, HHMMSS is the hour, minute, second, and FF is the frame number(00~19))"], data_dir: Annotated[str, "The directory of all videos"] = "./videomme") -> str:
    """Locate the video url based on the timestamp. The video is stored in the data_dir. We should return the video file path. Usually, the video name is DAYx_AN_NAME_HHMMSSFF.mp4"""
    ...
    try:
        ...
        timestamp="DAY1_"+timestamp.split("_")[-1]
        
        # Get all video files and sort them
        video_files = [f for f in os.listdir(data_dir) if f.endswith('.mp4')]
        video_files.sort()
        
        # Get the last video's timestamp (HHMMSSFF) from filename
        if not video_files:
            raise ValueError(f"No video files found in {data_dir}")
            
        last_video = video_files[-1]
        video_end_time = last_video.split('.')[0]  # Remove .mp4 extension
        
        # Map the input timestamp to the video range
        day_part, time_part = timestamp.split('_')
        time_part = scale_time_proportional(time_part, video_end_time)
        timestamp = f"{day_part}_{time_part}"
        day = day_part[3:]
        ...
        daily_videos = os.listdir(data_dir)
...
def locate_image_url(timestamp: Annotated[str, "The timestamp of the image to answer the question. The format should be DAYX_HHMMSSFF (X is the day number, HHMMSS is the hour, minute, second, and FF is the frame number(00~19))"], data_dir: Annotated[str, "The directory of all images"] = "/home/data2/sltian/code/Ego-R1_dev/EgoLife") -> str:
    """Locate the image url based on the timestamp. Get a screenshot from the video based on the timestamp."""
    video_path, exact_match = locate_video_url(timestamp=timestamp, data_dir=data_dir)
    ff = timestamp[6:8]
