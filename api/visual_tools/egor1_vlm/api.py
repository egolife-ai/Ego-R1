...
# Load configuration
config = load_config()
DATA_DIR = config['egolife']['vlm']['data_dir']
VLM_PORT = config['egolife']['vlm']['vlm_port']
...
async def vlm_endpoint(request: VLMRequest):
    ...
    """
    try:
        result = vlm(question=request.question, timestamp=request.timestamp, identity=request.identity,data_dir=DATA_DIR)
        return result
